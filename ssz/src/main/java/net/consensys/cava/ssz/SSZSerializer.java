...
import static net.consensys.cava.ssz.SSZSerializerEncode.encodeNextField;
...
public class SSZSerializer {
    ...
    static SSZType extractType(Class clazz, @Nullable String extra) {
        if (extra == null) {
            ...
            if (res != null) {
                ...
                return SSZType.of(SSZType.Type.CONTAINER);
            }
        }
        ...
        if (matcher.find()) {
            ...
            if (endNumber != null) {
                ...
                if (bitLength % Byte.SIZE != 0) {
                    String error = String.format("Size of field in bits should match whole bytes, found %s", extra);
            ...
            String error = String.format("Type annotation \"%s\" for class %s is not correct", extra, clazz.getName());
            throw new RuntimeException(error);
        }
    ...
    public static byte[] encode(Object input) {
        ...
        try {
            ...
            String error = String.format("Couldn't enumerate all getters in class %s", input.getClass().getName());
        ...
        for (SSZScheme.SSZField field : scheme.fields) {
            ...
            encodeNextField(value, field, res);
        }
    ...
    private static void checkSSZSerializableAnnotation(Class clazz) {
        if (!clazz.isAnnotationPresent(SSZSerializable.class)) {
            throw new RuntimeException("SSZ serializable class should be annotated with SSZSerializable!");
        }
    }
    ...
    public static Object decode(byte[] data, Class clazz) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            params[i] = field.type;
            values[i] = SSZSerializerDecode.decodeNextField(field, reader);
        }
        ...
        if (!constructorAttempt.getKey()) {
            ...
            if (!setterAttempt.getKey()) {
                ...
                String error = String.format("Unable to find appropriate class %s construction method with params [%s]." +
    ...
    private static Pair<Boolean, Object> createInstanceWithConstructor(Class clazz, Class[] params, Object[] values) {
    ...
    private static Pair<Boolean, Object> createInstanceWithSetters(Class clazz, List<SSZScheme.SSZField> fields, Object[] values) {
        ...
        for (int i = 0; i < fields.size(); ++i) {
            ...
            try {
                // Try to set by field assignment
                clazz.getField(currentField.name).set(result, values[i]);
            } catch (Exception e) {
                // Try to set using setter
                try {
